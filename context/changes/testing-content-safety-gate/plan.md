...
- [x] 4.5 Bramka przechodzi na prompcie z repo: `npm run test:gate` — po drugim doładowaniu (2026-09-02, saldo ~$6,27) przebieg **sekwencyjny** (`GATE_CONCURRENCY` tymczasowo 1, timeout tymczasowo 20 min — oba przywrócone do 3 / 10 min zaraz po zielonym wyniku) przeszedł czysto: 2 pliki, 6/6 testów, ~726 s, zero naruszeń, zero kolizji `402`. Dwie usterki znalezione i naprawione po drodze tego samego dnia: fałszywe trafienie na cytowanym „przepraszam" w treści (`content-safety-judge.ts`, `stripQuoted()`) i brak retry na `402 in_flight_budget_exhausted` z poziomu sędziego (`gate-retry.ts`, `retryGateCall()`) — obie pokryte bezpłatnymi testami jednostkowymi (`content-safety-judge.test.ts`, `gate-retry.test.ts`) — 277b601
- [x] 4.6 Raport wymienia każdy identyfikator z `allowed-models.ts` — asercja `expect(report).toContain(model)` w teście przeszła dla obu modeli — 277b601
- [x] 4.7 Bramka pokryła wszystkie cztery osiągalne tryby — asercja `expect(report).toContain(mode)` przeszła dla `day`, `day-weekday`, `day-themed`, `week` — 277b601
...
- [~] 4.13 `OPENROUTER_API_KEY` dodany jako repository secret; job przechodzi na prawdziwym PR-ze — sekret dodany (2026-09-02, `gh secret set`), job odpalił się i zużył go na PR #21, ale **nie przeszedł czysto**: 5/6 testów zielone, 1 awaria transportowa (szum, nie treść — patrz 4.5). PR zmergowany mimo tego, zgodnie z doradczym charakterem bramki (test-plan.md §6.5, plan.md #4)
- [x] 4.14 Job nie odpala się na PR-ze poza filtrem ścieżek — strona pozytywna potwierdzona: PR #21 (dotykający `content-safety-judge.ts` i in.) uruchomił job. Strona negatywna (PR niedotykający tych ścieżek) niesprawdzona bezpośrednio, ale filtr sam w sobie zweryfikowany statycznie w Fazie 4 (4.8)
